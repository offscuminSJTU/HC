...
from fs3d.ops import primitiveCLloss
...
class PrototypicalVoteModule(nn.Module):
    ...
    def forward(self, seed_points, seed_feats, prototypes=None, stage="train", gt_vote=None):
        ...
        # print('num_seeds: ', num_seed) ## 1024
        num_vote = num_seed * self.vote_per_seed

        x, prim_clloss, _ = self.point_refinement(seed_feats, xyz=seed_points, prototypes=prototypes, stage=stage)
        # TODO: return cl_loss here
        # prim_clloss = self.point_refinement(seed_feats, xyz=seed_points, prototypes=prototypes, stage=stage)[1]

        # (batch_size, (3+out_dim)*vote_per_seed, num_seed)
        middle_feature = seed_feats
        votes = self.vote_layer(x) # one layer of conv1d
        ...
        return vote_points, vote_feats, offset, middle_feature, seed_feats, prim_clloss
...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dropout=0.1, embedding_num=120):
        ...
        # self.init_weights()
        self.primitiveCLloss = primitiveCLloss(if_proj=True)
    ...
    def update_memory(self, features, info):
        ...
        features = features.permute(1, 0, 2).detach() # (1024, 16, 256) # 这里切断了梯度
        indices = info['fp_indices'] # (16, 1024)
        pts_semantic_masks = info['pts_semantic_mask'] # list[16: 20000]
        pts_instance_masks = info['pts_instance_mask']

        feature_list = [[] for i in range(self.memory_banks.shape[0])]
        # memory_bank: (120, 256); feature_list: [120]

        primitive_label = [[] for i in range(features.shape[0])] # record primitive label of one batch

        for batch_index in range(features.shape[0]):
            feature = features[batch_index] # (1024,256)
            indice = indices[batch_index] # (1024,)
            pts_semantic_mask = pts_semantic_masks[batch_index][indice] # (1024,) semantic mask of feature pts
            pts_instance_mask = pts_instance_masks[batch_index][indice] # not used

            foreground_indice = torch.nonzero(pts_semantic_mask < 18, as_tuple=False).squeeze(-1) # (87,) foreground pts
            foreground_features = feature[foreground_indice] # (87,256)
            foreground_semantic_mask = pts_semantic_mask[foreground_indice] # (87,)
            similarity_matrix = torch.mm(foreground_features, self.memory_banks.permute(1, 0)) # (87,120)

            max_indices = torch.argmax(similarity_matrix, dim=1) # (87,), 实际上没有使用
            max_all_indices = torch.sort(similarity_matrix, dim=1, descending=True)[1] # (87,120)

            feature_list_count = torch.zeros(len(feature_list)) # (120,)
            class_list_count = torch.zeros(18) # (18,)

            contain_num = int(max_all_indices.shape[0] / max_all_indices.shape[1]) + 1 # 抽屉原理

            primitive2feature = (torch.zeros(max_all_indices.shape[1], contain_num) - 1).long()

            for i in range(len(max_indices)): # for i in range(87), also ind in foreground

                label = foreground_semantic_mask[i] # semantic label
                if class_list_count[label] >= self.prototype_num_per_class: # self.prototype_num_per_class=10
                    # 每个class最多添加self.prototype_num_per_class个feature到memory bank中
                    continue
                class_list_count[label] += 1

                max_all_indice = max_all_indices[i] # (120,): ith feature pt & its most similar primitive prototypes
                indice_in_indice = torch.nonzero(feature_list_count[max_all_indice] < contain_num, as_tuple=False)[0]
                max_indice = max_all_indice[indice_in_indice] # max_indice: tensor([12], device='cuda:0')
                # print('max_indice', max_indice)
                # print('feature_list_count[max_indice].dtype:', feature_list_count[max_indice].dtype) # original is float32

                # TODO: record ind in seeds & ind in memory bank
                primitive2feature[max_indice, feature_list_count[max_indice].long()] = foreground_indice[i]

                feature_list_count[max_indice] += 1
                foreground_feature = foreground_features[i].unsqueeze(0) # (1,256)
                feature_list[max_indice].append(foreground_feature)

            primitive_label[batch_index] = primitive2feature
            # print('foreground pt_num: ', len(max_indices))
            # print(primitive2feature)
        ...
        for index in range(len(feature_list)):
            ...
            one_slot = torch.mean(one_slot, dim=0) # (256)
            # print('one_slot: ', one_slot.shape)
            features_norm = torch.norm(one_slot, p=2, dim=0)
            one_slot = one_slot.div(features_norm) # 归一化feature_list
            # print('one_slot: ', one_slot.shape, one_slot)
            self.memory_banks[index] = self.mu * self.memory_banks[index] + (1 - self.mu) * one_slot # 动量式更新memory_bank

        features_norm = torch.norm(self.memory_banks_embedding.weight, p=2, dim=1).unsqueeze(1) # 更新完了再归一化
        self.memory_banks_embedding.weight[:] = self.memory_banks_embedding.weight.div(features_norm)
        # TODO: return primitive labels & their pt indices
        #  拉近相同primitive label pt feature, 推远不同primitive label pt feature
        #  return a (120 * pt_num) matrix
        # print('primitive_label: ', len(primitive_label))
    ...
    def forward_pre(self, tgt, memory=None,
        ...
        if isinstance(prototypes, dict): # pretraining only
            primitivelabel = self.update_memory(q, prototypes)
            # print('memory_bank: ', self.memory_banks_embedding.weight)
            prim_clloss = self.primitiveCLloss(primlabel=primitivelabel, features=q, prototype=self.memory_banks_embedding.weight)
            # print('prim_loss: ', prim_clloss) # 4 if weight=1
            # TODO: return here
