...
from fs3d.core.post_processing import aligned_3d_nms
from fs3d.models.builder import build_loss
from fs3d.models.losses import chamfer_distance
from fs3d.models.model_utils import PrototypicalVoteModule
from fs3d.ops import build_sa_module, furthest_point_sample
...
class VoteHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pv_module_cfg=None,
                 aggregation_ph_module_cfg=None,
                 pred_layer_cfg=None,
        ...
        self.gt_per_seed = pv_module_cfg['gt_per_seed']
        self.num_proposal = aggregation_ph_module_cfg['num_point']
        ...
        self.att_class_loss = build_loss(att_class_loss)
        ...
        self.PrototypicalVoteModule = PrototypicalVoteModule(**pv_module_cfg)

        self.aggregation_ph_refinement = build_sa_module(aggregation_ph_module_cfg)
    ...
    def forward(self, feat_dict, sample_mod, prototypes=None, cls_prototypes=None, stage="train", gt_vote=None):
        """Forward pass.
        ...
        vote_points, vote_features, vote_offset, attention_features, before_attention_features, prim_clloss = self.PrototypicalVoteModule(
            seed_points, seed_features, prototypes=prototypes, stage=stage, gt_vote=gt_vote)
        ...
        results = dict(
            ...
            before_attention_features=before_attention_features)

        # print('sample_mod: ', sample_mod) ## vote
        # 2. aggregate vote_points
        if sample_mod == 'vote':
            # use fps in vote_aggregation
            aggregation_inputs = dict(
                points_xyz=vote_points, features=vote_features, seed_xyz=seed_points, cls_prototypes=cls_prototypes, seed_indices=seed_indices)
        ...
        vote_aggregation_ret = self.aggregation_ph_refinement(**aggregation_inputs)
        # print()

        aggregated_points, features, aggregated_indices, cl_loss = vote_aggregation_ret
        # print('cl_loss: ', cl_loss)
        # TODO: add prim_clloss here
        #  use prim_clloss only during debugging
        if prim_clloss is not None:
            cl_loss = cl_loss + prim_clloss # weight control
            # cl_loss = cl_loss / 2

        results['aggregated_points'] = aggregated_points
        results['aggregated_features'] = features
        results['aggregated_indices'] = aggregated_indices
        # results['target_prototypes'] = target_prototypes
        ...
        results['contrastive_loss'] = cl_loss
    ...
    def loss(self,
        ...
        cl_loss = bbox_preds.pop('contrastive_loss')
        ...
        # calculate vote loss
        vote_loss = self.PrototypicalVoteModule.get_loss(bbox_preds['seed_points'],
        ...
        # print("heading_label_one_hot: " + str(heading_label_one_hot.shape))
        ...
        # print("size_residual_norm: " + str(size_residual_norm.shape))
        # print("size_res_targets: " + str(size_res_targets.shape))
        # print("box_loss_weights_expand: " + str(box_loss_weights_expand.shape))
        ...
        # print("bbox_preds['sem_scores'].transpose(2, 1): " + str(bbox_preds['sem_scores'].transpose(2, 1).shape))
        # print("mask_targets: " + str(mask_targets.shape))
    ...
    def get_targets(self,
        ...
        # print("aggregated_points: " + str(aggregated_points[0].shape))
    ...
    def get_targets_single(self,
        ...
        if self.bbox_coder.with_rot:
            ...
            # print("pts_semantic_mask is not None:")
            vote_targets = points.new_zeros([num_points, 3])
            vote_target_masks = points.new_zeros([num_points],
                                                 dtype=torch.long)
            # print("pts_semantic_mask: " + str(pts_semantic_mask.shape))
            # print("pts_instance_mask: " + str(pts_instance_mask.shape))
            for i in torch.unique(pts_instance_mask):
                ...
                if pts_semantic_mask[indices[0]] < self.num_classes:
                    ...
                    vote_target_masks[indices] = 1
            # print("vote_targets: " + str(vote_targets.shape))
        ...
        # print("mean_sizes:" + str(mean_sizes.shape))
        # print("one_hot_size_targets: " + str(one_hot_size_targets.shape))

        pos_mean_sizes = torch.sum(one_hot_size_targets * mean_sizes, 1)
        size_res_targets /= pos_mean_sizes

        # print("pos_mean_sizes: " + str(pos_mean_sizes.shape))
        # print("size_res_targets: " + str(size_res_targets.shape))
